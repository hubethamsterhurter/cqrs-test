...
export const ChatHistory: React.FC = function ChatHistory(props) {
  ...
  }, [ dataCtx.state.cache.chats.byId, forceScrollBottom, chatsRef, ]);


  return (
    <div ref={chatsRef} onScroll={handleScroll} className="chat-history">
      {dataCtx.state.cache.chats.ids.map((chatId: string) => {
        const chat = dataCtx.state.cache.chats.byId[chatId];
        const author: UserModel | null = chat.author_id ? dataCtx.state.cache.users.byId[chat.author_id] : null;
